...
export class CartController {
  ...
  async checkout(@Req() req, @Res() res, @Body() body) {
    ...
    const userId = req.session.user.id;
    const { name, phone, address, payment, note } = body;

    const cart = await this.cartService.findByUser(userId);

    // Giỏ trống thì không tạo đơn rỗng
    if (!cart.length) {
      return res.redirect('/cart');
    }

    // 📝 Lưu đơn hàng vào database để Admin có dữ liệu quản lý & thống kê
    const order = await this.ordersService.createOrder({
      user_id: userId,
      customerName: String(name || '').trim(),
      phone: String(phone || '').trim(),
      address: String(address || '').trim(),
      paymentMethod: payment === 'bank' ? 'bank' : 'cash',
    ...
    return res.send(`
      ...
      ">
        ...
        <p><b>Tổng tiền:</b> ${Number(order.total).toLocaleString('vi-VN')} VNĐ</p>
